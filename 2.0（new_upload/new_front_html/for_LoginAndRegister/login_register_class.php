...
include_once "user.php";
    class login_register_class{
        function for_login($user,$password){
            ...
            if($user==$root&&$password==$root_pass){
                ...
                $person=new user();
                $result=$person->login($user,$password);
                if ($result<0){
                    echo "<script type='text/javascript'>alert('没有这个用户');location='javascript:history.back()';</script>";
                }
                else if($result==0){
                    ...
                    echo "<script type='text/javascript'>alert('登录成功');location='javascript:history.back()';</script>";
        ...
        function for_register($username,$pass,$email,$qq,$safe){
           /* $conn = mysqli_connect("localhost", "root", "root", "db_database10") or die("连接数据库服务器失败！".mysqli_error());
            mysqli_query($conn,"set names 'utf8'");
            $user=$username;
            $password=$pass;
            ...
            if($person->checkUsername($username)){
                if($person->insertinfo($username,$pass,$email,$qq,$safe)){
                    echo "<script type='text/javascript'>alert('注册成功');location='javascript:history.back()';</script>";
                }
                else{
                    echo "<script type='text/javascript'>alert('注册失败');location='javascript:history.back()';</script>";
                }
            }
            else{
                echo "<script type='text/javascript'>alert('用户名已存在，请重新写');location='javascript:history.back()';</script>";
        ...
        function for_find($user,$pin,$password){
            ...
            $person=new user();
            if($person->reback($user,$pin,$password)<0){
                echo "<script type='text/javascript'>alert('用户名不存在');location='javascript:history.back()';</script>";
            }
            else if ($person->reback($user,$pin,$password)==0){
                echo "<script type='text/javascript'>alert('PIN不正确');location='javascript:history.back()';</script>";
            }
            else{
                echo "<script type='text/javascript'>alert('修改成功，2秒进入登录界面');</script>";
